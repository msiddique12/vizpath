#!/usr/bin/env python3
"""Run the code analysis agent example.

Usage:
    # Set your API key
    export NVIDIA_API_KEY="your-api-key"

    # Run the agent (from vizpath root directory)
    python -m examples.code_agent.run "How does the intelligence module work?"

    # Analyze a specific directory
    python -m examples.code_agent.run "What are the main components?" --codebase ./server

    # Verbose mode
    python -m examples.code_agent.run "Explain the tracing system" -v
"""

import argparse
import os
import sys

from vizpath import configure, tracer

from .agent import CodeAnalysisAgent


def main():
    parser = argparse.ArgumentParser(
        description="Code Analysis Agent - Analyzes codebases using Vizpath tracing",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument(
        "question",
        nargs="?",
        default="How does the intelligence module work?",
        help="The question to answer about the codebase",
    )
    parser.add_argument(
        "--codebase",
        default=".",
        help="Root directory of the codebase to analyze (default: current directory)",
    )
    parser.add_argument(
        "--api-key",
        help="NVIDIA API key (or set NVIDIA_API_KEY env var)",
    )
    parser.add_argument(
        "--model",
        default="nvidia/llama-3.3-nemotron-super-49b-v1.5",
        help="Model to use (default: nvidia/llama-3.3-nemotron-super-49b-v1.5)",
    )
    parser.add_argument(
        "--vizpath-url",
        default="http://localhost:8000",
        help="Vizpath server URL (default: http://localhost:8000)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Run without sending traces to vizpath (for testing)",
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
        help="Show detailed progress during analysis",
    )
    parser.add_argument(
        "--max-iterations",
        type=int,
        default=8,
        help="Maximum analysis iterations (default: 8)",
    )

    args = parser.parse_args()

    # Configure vizpath
    if not args.dry_run:
        configure(api_url=args.vizpath_url)
        print(f"Vizpath configured: {args.vizpath_url}")
    else:
        print("Dry run mode - traces will not be sent to vizpath")

    # Check for API key
    api_key = args.api_key or os.getenv("NVIDIA_API_KEY")
    if not api_key:
        print("Error: NVIDIA_API_KEY not set. Please set it via:")
        print("  export NVIDIA_API_KEY='your-api-key'")
        print("  or pass --api-key 'your-api-key'")
        sys.exit(1)

    print()
    print("Code Analysis Agent")
    print("=" * 50)
    print(f"Question: {args.question}")
    print(f"Codebase: {args.codebase}")
    print(f"Model: {args.model}")
    print()

    # Create and run agent
    agent = CodeAnalysisAgent(
        api_key=api_key,
        model=args.model,
        max_iterations=args.max_iterations,
        verbose=args.verbose,
    )

    print("Starting analysis...\n")
    answer = agent.analyze(args.question, args.codebase)

    print()
    print("=" * 50)
    print("ANSWER")
    print("=" * 50)
    print(answer)
    print("=" * 50)

    # Flush any pending spans and show stats
    tracer.flush()
    stats = tracer.stats()

    if not args.dry_run:
        print(f"\nTracing: {stats['traces']} traces, {stats['spans']} spans sent")
        print("\nView traces at: http://localhost:3000/traces")
    elif args.verbose:
        print(f"\nTracing stats (dry run): {stats['traces']} traces, {stats['spans']} spans")


if __name__ == "__main__":
    main()

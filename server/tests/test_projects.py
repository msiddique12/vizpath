...
class TestProjectList:
    ...
    def test_list_projects_requires_authentication(self, client, test_db, monkeypatch):
        """GET /projects requires a valid API key."""
    ...
    def test_list_projects_returns_only_current_project(self, client, test_db):
        """GET /projects should be tenant-scoped, not global enumeration."""
        project_1_key = client.post("/api/v1/projects/", json={"name": "project-1"}).json()["api_key"]
        client.post("/api/v1/projects/", json={"name": "project-2"})

        response = client.get("/api/v1/projects/", headers={"X-API-Key": project_1_key})
    ...
    def test_list_projects_does_not_include_api_key(self, client, test_db):
        ...
        api_key = client.post("/api/v1/projects/", json={"name": "secret-project"}).json()["api_key"]
